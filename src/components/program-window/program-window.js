...
class ProgramWindow extends Component {
  ...
  defaultWindow() {
    return this.props.program.isFullscreen
      ? this.regularWindow()
      ...
      <div
        className={
          this.props.program.isFullscreen
            ...
            : 'regular-window window'
        }
      >
        <WindowBar
          closeAction={() => this.closeWindow()}
          minimizeAction={() => this.minimizeWindow()}
          maximizeAction={() => this.maximizeWindow()}
          isDragable={!this.props.program.isFullscreen}
        />
  ...
  draggableWindow() {
    return <Draggable handle=".handle">{this.regularWindow()}</Draggable>;
  }
}
...
const mapDispatchToProps = dispatch => ({
  ...
  fullscreenProgram: (id, windowId, isFullscreen) =>
    dispatch(fullscreenProgram(id, windowId, isFullscreen)),
});
